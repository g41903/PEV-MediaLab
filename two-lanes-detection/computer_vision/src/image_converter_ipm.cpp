...
class ImageConverter
{
  ...
  int width, height;
  double prev_degree, prev_distance;
...
//        showChannels(outputImg);
        ...
        threshold( src_gray, src_binary, 50, 255, THRESH_BINARY );
...
//        src_binary = extractWhitePixel(src_gray, 2, 1);
  ...
  void drawLines(Mat roi, vector<Vec2f> lines)
  {
      ...
      double avg_right_rho = 0;
      double avg_right_theta = 0;
...
//          if ((degree >= 0 && degree <= 80) || (degree >= 160 && degree <= 180))
//          {
...
//          }
      ...
      double degree2 = 0;
      double right_degree = 0;

      for( size_t i = 0; i < lines.size(); i++ )
      {

          double rho2 = lines[i][0];
          double theta2 = lines[i][1];

          degree2 = theta2 * 180 / CV_PI;
          if(degree2 > 90)
          {
              degree2-=180;
          }

          cout<<"rho "<< i <<":" << rho2 <<endl;
          cout<<"theta "<< i <<":"<< theta2 <<endl;


          right_rho += abs(rho2);
          right_degree += degree2;
//          right_theta += theta2;
      }

      cout<<"sum_right_rho : "<< right_rho <<endl;
      cout<<"sum_right_theta : "<< right_degree * CV_PI / 180 <<endl;
      cout<<"lines.size : "<< lines.size() <<endl;

      avg_right_rho = right_rho / lines.size();
      avg_right_theta = (right_degree * CV_PI / 180) / lines.size();

      cout<<"avg_right_rho : "<< avg_right_rho <<endl;
      cout<<"avg_right_theta : "<< avg_right_theta <<endl;
      ...
      distance = abs(avg_right_rho);
      degree = avg_right_theta * 180 / CV_PI;

      if (lines.size() > 0 && (degree >= 0 && degree <= 90) ||  degree >= -90 && degree < 0)
      {
      ...
      if(lines.size() == 0)
      {
          ...
          lane_msg.rho = prev_distance;
          lane_msg.theta = prev_degree;
          lane_pub.publish(lane_msg);
      }
